...
export function asyncActionHandlerFactory<PayloadType, ReturnedType, ErrorType>(
  ...
  const effect = (
    ...
    requestFn: (payload: PayloadType) => Observable<ReturnedType>,
  ) =>
    action$.pipe(
      ...
      concurrencyOperator((action: PayloadMetaAction<PayloadType>) =>
        requestFn(action.payload).pipe(
          ...
          takeUntil(
            action$.pipe(
              filter((cancelAction: Action) => cancelAction.type === ActionTypes.ACTION_CANCELED),
            ),
          ),
        ),
      ),
...
function createActions<PayloadType, ReturnedType, ErrorType, CancelType>(type: string) {
  ...
  const Actions = {
    ...
    success: <P>(payload: P, meta?: string) => ({
      type: ActionTypes.ACTION_SUCCEEDED,
      payload,
      meta,
    }),
    failure: <P>(payload: P, meta?: string) => ({ type: ActionTypes.ACTION_FAILED, payload, meta }),
    cancel: <P>(payload?: P, meta?: string) => ({
      type: ActionTypes.ACTION_CANCELED,
      payload,
      meta,
    }),
  };
